...
<form action="" method="POST" style="background-color:white;opacity:0.8;">
		...
		<input type="email" id="mail" name="emailid" placeholder="  Type Here"><br><br>
...
if(isset($_POST['submit']))
{
	...
	$query="select emailid from student where emailid='$emailid'";
	$execute=mysqli_query($conn,$query);

	if(mysqli_num_rows($execute)>0)
	{	
   
	$updatequery="update student set studentpassword='$password' where emailid='$emailid'";
	
	if(mysqli_query($conn,$updatequery))
	{
		echo "<script>alert('Updated Successfully!');window.location = 'home.php'</script>";
	}
	else{
		echo "<script>alert('Something wrong');window.location = 'studentforgotpassword.php'</script>";
	}
}
else{
	echo "<script>alert('Check Your Email id !');window.location = 'studentforgotpassword.php'</script>";
}
}
?>